...
Nutanix veri toplama tablosu (discovery_nutanix_inventory_host) karşılaştırılarak
...
# Netbox device ve Nutanix host tablolarını serial bazlı karşılaştıran sorgu
# Netbox: tags1_display = 'ESXI NUTANIX' AND tenant_name = 'Bulutistan - Virtualization'
# Nutanix: serial UNIQUE olmadığından DISTINCT ON kullanılır
DETECTION_QUERY = """
...
SELECT
    ...
    CASE
        WHEN nut.serial IS NULL THEN 'MISSING'
...
ORDER BY
    CASE WHEN nut.serial IS NULL THEN 0 ELSE 1 END,
...
INSERT INTO hmdl.hmdl_datalake_monitoring_nutanix_host (
    ...
    netbox_device_name,
    netbox_site_name,
    netbox_location_name,
    netbox_rack_name,
    netbox_status_value,
    netbox_serial,
    netbox_device_type_name,
    netbox_manufacturer_name,
    netbox_device_role_name,
    netbox_tenant_name,
    netbox_primary_ip_address,
    netbox_tags1_display,
    netbox_total_cores,
    netbox_total_ram,
    netbox_cpu,
...
# INSERT için değer şablonu
INSERT_TEMPLATE = """(
    %(check_time)s,
    %(check_threshold_hours)s,
    %(netbox_device_id)s,
    %(netbox_device_name)s,
    %(netbox_site_name)s,
    %(netbox_location_name)s,
    %(netbox_rack_name)s,
    %(netbox_status_value)s,
    %(netbox_serial)s,
    %(netbox_device_type_name)s,
    %(netbox_manufacturer_name)s,
    %(netbox_device_role_name)s,
    %(netbox_tenant_name)s,
    %(netbox_primary_ip_address)s,
    %(netbox_tags1_display)s,
    %(netbox_total_cores)s,
    %(netbox_total_ram)s,
    %(netbox_cpu)s,
...
def find_stale_and_missing_hosts(conn, threshold_hours):
    """
    Netbox envanterindeki Host'ları Nutanix veri toplama tablosuyla
    karşılaştırarak STALE ve MISSING Host'ları tespit eder.
    """
    logger.info(
...
def save_results(conn, results, check_time, threshold_hours):
    """Tespit edilen Host'ları hmdl.hmdl_datalake_monitoring_nutanix_host tablosuna yazar."""
    if not results:
...
def print_summary(results):
    ...
    logger.info("=" * 60)
    logger.info("KONTROL SONUCU - NUTANIX HOST")
    logger.info("=" * 60)
    ...
    if missing:
        logger.info("")
        logger.info("MISSING Host'lar (Nutanix'te hiç kaydı yok):")
        for host in missing[:20]:
            logger.info(
                "  - %-40s  [serial: %s, site: %s, rack: %s]",
                host["netbox_device_name"],
                host.get("netbox_serial", "-"),
                host.get("netbox_site_name", "-"),
                host.get("netbox_rack_name", "-"),
            )
        if len(missing) > 20:
    ...
    if stale:
        logger.info("")
        logger.info("STALE Host'lar (1 haftadan uzun süredir veri yok):")
        for host in stale[:20]:
            ...
            logger.info(
                "  - %-40s  [serial: %s, son veri: %s, yaş: %s gün]",
                host["netbox_device_name"],
                host.get("netbox_serial", "-"),
                host.get("nutanix_last_observed", "-"),
                age_days,
            )
        if len(stale) > 20:
